...
from tools.commoncrawl_tool import CommonCrawlTool
...
class ReconAgent:
    ...
    def __init__(
        ...
        self.wayback = WaybackTool()
        self.commoncrawl = CommonCrawlTool()
        self.otx = OTXTool()
    ...
    def scan(self, target, host_limit=25):
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            futures = {
                executor.submit(self._scan_host, host): host.host
            ...
            for future in as_completed(futures):
                completed += 1
                host = future.result()
                print(f"[{completed}/{total}] {host.status:3}  {host.host}")
        ...
        crawl_hosts = [
            ...
            if host.status in (200, 201, 202, 203, 204, 205, 206, 207, 208, 226)
        ]

        # -----------------------------------
        # Katana Crawl + downstream analysis
        # -----------------------------------
        try:
            print()
            ...
            # -----------------------------------
            # Parallel Katana Crawling
            # -----------------------------------

            total_crawl_hosts = len(crawl_hosts)

            print(
                f"Starting parallel crawl of "
                f"{total_crawl_hosts} hosts..."
            )

            #
            # Use the configured scanner workers,
            # but don't create more Katana processes
            ...
            if crawl_workers < 1:
                ...
                print(
                    "No live hosts available for Katana."
                )
            ...
            with ThreadPoolExecutor(max_workers=3) as executor:
                ...
                for future in as_completed( futures):
                    ...
                    if source == "wayback":
                        ...
                        result.wayback_urls = len(urls)

                        result.archived_urls = len(urls)

                    elif source == "commoncrawl":

                        result.commoncrawl_urls = len(urls)

                    elif source == "otx":

                        result.otx_urls = len(urls)
            ...
            print()
            print(f"Unique URLs : {len(result.urls)}")
            print(f"Duplicates Removed : {result.duplicate_urls}")
                        # -----------------------------------
                        # robots.txt
                        # -----------------------------------
            print()
            ...
            # Add robots.txt paths to URL database
            for path in result.robots.interesting_paths:
            ...
            # Add sitemap URLs to URL database
            ...
            if result.sitemap.found:
                existing = {u.url for u in result.urls}
                added = 0
                for sitemap_url in result.sitemap.urls:
                    if sitemap_url in existing:
                        continue
                    result.urls.append(UrlResult(url=sitemap_url))
            ...
            result.url_categories = self.url_classifier.classify(result.urls)
            ...
            print("-" * 35)
            expected = [
                "API", "Login", "Admin", "Dashboard", "Auth",
                "Static", "Images", "Downloads", "Documents", "Unknown",
            ]
            for category in expected:
                print(f"{category:<15}{summary.get(category, 0):>5}")
            print("-" * 35)
            print(f"{'Total URLs':<15}{len(result.url_categories):>5}")
            ...
            xfo = sum(h.x_frame_options for h in result.header_results)
            xcto = sum(h.x_content_type_options for h in result.header_results)
            referrer = sum(h.referrer_policy for h in result.header_results)
            permissions = sum(h.permissions_policy for h in result.header_results)
            ...
            for title, attribute in critical_headers:
                missing = [
                    h.host for h in result.header_results
            ...
            result.tls_results = []
            for host in crawl_hosts:
                result.tls_results.append(self.tls.analyze(host.host))
            ...
            for tls in result.tls_results:
                ...
                print(
                    f"  Public Key  : "
                    f"{tls.public_key_algorithm} {tls.key_size}"
                )
                print(f"  Expires     : {tls.days_remaining} days")
                print(f"  Wildcard    : {tls.wildcard}")
                print(f"  Fingerprint : {tls.certificate_fingerprint}")
            ...
            for host in crawl_hosts:
                result.http_methods.append(
                    self.http_methods.analyze(f"https://{host.host}")
                )
            ...
            for item in result.http_methods:
                ...
                if item.dangerous:
                    print("  Dangerous : " + ", ".join(item.dangerous))
                print()
           ...
           # -----------------------------------
            ...
            print()
                      ...
                      #
                      # Only send likely JavaScript files to the scanner
                      #
            ...
            for url in result.urls:
                ...
                lower = url.url.lower().split("?", 1)[0]
          
                      #
                      # Ignore obvious non-JS assets
                      #
            ...
            print(
                f"Scanning {len(js_candidates)} JavaScript candidates..."
            )
            print()
          
            result.javascript_files = self.javascript.scan(
                js_candidates
            )
            ...
            for js in result.javascript_files[:10]:
          
                print(f"  {js.url}")
          
            if len(result.javascript_files) > 10:
          
                print(
                    f"\n... and {len(result.javascript_files)-10} more JavaScript files"
                )
                          # -----------------------------------
                          # JavaScript Analysis
                          # -----------------------------------
            ...
            result.javascript_findings = self.javascript_analyzer.analyze(
            ...
            expected = [
                "URL", "REST Endpoint", "GraphQL Endpoint", "WebSocket",
                "Email", "TODO/FIXME", "JWT Token", "Google API Key",
                "Firebase URL", "AWS Key", "Internal IP", "Authorization Header",
            ]
            ...
            for finding_type in expected:
               print(
                    f"{finding_type:<25}"
                    f"{summary.get(finding_type, 0):>5}"
                )
            ...
            print(
                f"{'Total Findings':<25}"
                f"{len(result.javascript_findings):>5}"
            )
            # -----------------------------------
            # JWT Analysis
            # -----------------------------------
            ...
            for finding in result.javascript_findings:
                if finding.type != "JWT Token":
                    continue
                jwt = self.jwt.decode(finding.value, finding.source)
                result.jwt_results.append(jwt)

            print(f"Decoded {len(result.jwt_results)} JWTs")
